...
class MainActivity : AppCompatActivity() {
     ...
     */
    private val LEMONADE_STATE = "LEMONADE_STATE" // Estado de la limonada
    private val LEMON_SIZE = "LEMON_SIZE" // Tamaño de limon?
    private val SQUEEZE_COUNT = "SQUEEZE_COUNT" // Recuento de exprimiciones
    ...
    private var lemonTree = LemonTree() // metodo pick 2 al 4
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        {
            Log.e("Saved instances state", "null")
        }
        // === END IF STATEMENT ===

        lemonImage = findViewById(R.id.image_lemon_state)
        setViewElements()
        lemonImage!!.setOnClickListener { //Metodo para actualizar el estado
            clickLemonImage()
            setViewElements()
            showSnackbar()
        }
    ...
    private fun clickLemonImage() {
        // Log.w("mispe", "${lemonadeState}")
        when(lemonadeState){
            "SELECT" -> {
                // Log.w("lemonadeState", "Entro al SELECT")
                lemonadeState = "SQUEEZE"
                lemonSize = lemonTree.pick()
                squeezeCount = 0
            }
            "SQUEEZE" -> {
                // Log.w("lemonadeState", "Entro al SQUEEZE")
                squeezeCount++
                lemonSize--
                if(lemonSize == 0){
                    lemonadeState = "DRINK"
            ...
            "DRINK" -> {
                // Log.w("lemonadeState", "Entro al DRINK")
                lemonadeState = "RESTART"
                lemonSize = -1
            }
            "RESTART" -> {
                // Log.w("lemonadeState", "Entro al RESTART")
